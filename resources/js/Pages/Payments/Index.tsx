...
export default function Payments() {
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await axios.get(`/api/students/search?nis=${searchNis}`);
            setStudent(res.data.student);
            setUnpaidBills(res.data.unpaid_bills);
            if (res.data.unpaid_bills.length === 0) {
                showNotification('success', 'Tidak Ada Tagihan', 'Siswa ini tidak memiliki tagihan yang belum dibayar.');
            }
        } catch (error: any) {
            showNotification('error', 'Tidak Ditemukan', error.response?.data?.error || 'Siswa tidak ditemukan.');
        } finally {
        ...
        const processBills = async () => {
            ...
            if (successCount > 0) {
                ...
                const paidBillNames = unpaidBills
                    .filter(b => selectedBillIds.includes(b.id))
                    .map(b => b.bill_type.name + (b.period ? ` (${b.period})` : ''))
                    .join(', ');
                ...
                showNotification('success', 'Pembayaran Berhasil!', `Tanda terima untuk Rp ${processedAmount.toLocaleString('id-ID')} siap dicetak.`);
                setShowReceipt(true);
                
                // Refresh list
                handleSearch();
            } else {
                showNotification('error', 'Gagal', 'Terjadi kesalahan saat memproses pembayaran.');
            }
        };
        ...
        <DashboardLayout>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 print:p-0 print:bg-white print:static print:z-auto">
                    <div className="bg-white text-black w-full max-w-md rounded-2xl shadow-2xl overflow-hidden print:shadow-none print:max-w-full">
                        ...
                        <div className="p-6 border-b-2 border-dashed border-gray-300 relative text-center">
                            ...
                            <h2 className="font-bold text-lg leading-tight uppercase">SMA Negeri 1</h2>
                            <p className="text-xs text-gray-500 mb-4">Jl. Pendidikan No. 123, Kota Pelajar</p>
                            ...
                            <p className="text-sm font-mono text-gray-600">{lastTransaction.id}</p>
                        </div>
                        ...
                        <div className="p-6 space-y-4 text-sm relative">
                            ...
                            <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-[0.03]">
                                <span className="text-6xl font-black rotate-[-30deg] uppercase tracking-widest text-black">PAID</span>
                            </div>

                            <div className="flex justify-between">
                                <span className="text-gray-500">Tanggal</span>
                                <span className="font-semibold text-right">{lastTransaction.date}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-500">Nama Siswa</span>
                                <span className="font-semibold text-right">{lastTransaction.student}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-500">NIS / Kelas</span>
                                <span className="font-semibold text-right">{lastTransaction.nis} / {lastTransaction.class}</span>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mb-2">{lastTransaction.months}</p>
                            </div>
                            ...
                            <div className="flex justify-between items-center text-lg">
                                ...
                                <span className="font-black text-green-600">Rp {lastTransaction.amount.toLocaleString('id-ID')}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 p-6 flex items-center justify-between border-t border-gray-200">
                            <div className="pr-4">
                                <p className="text-[10px] text-gray-500 font-medium leading-tight mb-1">
                                    Dokumen ini diamankan dengan Tanda Tangan Digital. Pindai QR Code untuk memverifikasi keaslian bukti pembayaran pada sistem resmi sekolah.
                                </p>
                            </div>
                            <div className="shrink-0 bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
                                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=https://sikola.com/verify/${lastTransaction.id}&margin=0`} alt="Verification QR" className="w-16 h-16" />
                            </div>
                        </div>
                        
                        {/* Actions (Hidden on Print) */}
                        <div className="p-4 bg-white border-t border-gray-100 flex gap-3 print:hidden">
                            <button onClick={() => setShowReceipt(false)} className="flex-1 py-2.5 px-4 rounded-xl text-gray-600 font-semibold hover:bg-gray-100 transition-colors">Tutup</button>
                            <button onClick={handlePrint} className="flex-1 py-2.5 px-4 rounded-xl bg-primary text-white font-bold hover:bg-primary/90 flex items-center justify-center transition-colors">
            ...
            <div className="max-w-4xl mx-auto w-full space-y-6 print:hidden">
                <div className="flex justify-between items-center mb-2">
                    <h2 className="font-headline-lg text-on-surface">Kasir Pembayaran</h2>
                    {lastTransaction && (
                        <button 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-surface rounded-2xl p-6 border border-outline-variant shadow-sm">
                            <h3 className="font-headline-md mb-4 text-on-surface">Cari Siswa</h3>
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Nomor Induk (NIS)</label>
                                ...
                                <button 
                                    ...
                                    {isSearching ? <span className="material-symbols-outlined animate-spin mr-2">progress_activity</span> : 'Cari Data'}
                                </button>
                            </div>
                        </div>
                        ...
                        {student && (
                            <div className="bg-primary-fixed/20 rounded-2xl p-6 border border-primary-fixed shadow-sm">
                                <div className="flex items-center space-x-4 mb-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-on-surface text-lg leading-tight">{student.name}</h4>
                                        <p className="text-sm text-on-surface-variant">{student.student_class?.name || '-'}</p>
                                    </div>
                                </div>
                                <div className="space-y-2 pt-4 border-t border-primary-fixed-dim/30 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-on-surface-variant">Status:</span>
                                        <span className="font-semibold text-primary">{student.status}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-on-surface-variant">Total Tagihan:</span>
                                        <span className="font-semibold text-error">
                                            Rp {unpaidBills.reduce((acc, curr) => acc + Number(curr.amount), 0).toLocaleString('id-ID')}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <h3 className="font-headline-md mb-6 text-on-surface flex justify-between items-center">
                                ...
                                {unpaidBills.length > 0 && (
                                    <div className="flex gap-3">
                                        {unpaidBills.filter(b => b.bill_type?.name?.toUpperCase() === 'SPP').length >= 6 && (
                                            <button 
                                                type="button" 
                                                onClick={() => selectSppMonths(6)}
                                        ...
                                        {unpaidBills.filter(b => b.bill_type?.name?.toUpperCase() === 'SPP').length >= 12 && (
                                            <button 
                                                type="button" 
                                                onClick={() => selectSppMonths(12)}
                                                className="text-xs font-semibold text-primary bg-primary-container px-3 py-1.5 rounded-lg hover:bg-primary/20 transition-colors">
                                                Lunas 1 Tahun
                                            </button>
                                        )}
                            ...
                            <form onSubmit={handlePayment} className="space-y-6">
                                ...
                                {unpaidBills.length === 0 ? (
                                    <div className="text-center p-8 bg-surface-container-lowest rounded-xl border border-outline-variant border-dashed">
                                        ...
                                        <p className="text-on-surface-variant font-medium">Tidak ada tagihan yang belum dibayar.</p>
                                    </div>
                                ) : (
                                            ...
                                            >
                                                <div className="flex items-center gap-4">
                                                    <div className={cn(
                                                        ...
                                                        selectedBillIds.includes(bill.id) ? "bg-primary border-primary text-on-primary" : "border-outline"
                                                    )}>
                                                        {selectedBillIds.includes(bill.id) && <span className="material-symbols-outlined text-[14px] font-bold">check</span>}
                                                    </div>
                                                    <div>
                                                        <p className="font-semibold text-on-surface">{bill.bill_type?.name} {bill.period ? `(${bill.period})` : ''}</p>
                                                        <p className="text-xs text-on-surface-variant">Tenggat: {new Date(bill.due_date).toLocaleDateString('id-ID')}</p>
                                                    </div>
                                                </div>
                                ...
                                {unpaidBills.length > 0 && (
                                    <>
                                        <div>
                                            <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2 mt-4">Metode Pembayaran</label>
                                            <select 
                                        ...
                                        <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant flex justify-between items-center mt-6">
                                            <div>
                                                <p className="text-sm font-medium text-on-surface-variant mb-1">Total yang akan dibayar</p>
                                            ...
                                            >
                                                ...
                                                {!isProcessing && <span className="material-symbols-outlined ml-2 text-[20px]">payments</span>}
                                            </button>
                                        </div>
                                    </>
                                )}
